test_CheckAttributes <- function() {
    ...
    checkException(res <- CheckAttributes(g, vertex.attr=c("attr1", "attr2")), silent=T)
    ...
    values[1] <- "a"
    g <- set.vertex.attribute(g, name="vertex.attr", value=values)
    checkException(CheckAttributes(g, vertex.attr="vertex.attr"), silent=T)
    g <- remove.vertex.attribute(g, "vertex.attr")
    ...
    values[1] <- -1
    g <- set.vertex.attribute(g, name="vertex.attr", value=values)
    checkException(res <- CheckAttributes(g, vertex.attr="vertex.attr"), silent=T)
    g <- remove.vertex.attribute(g, "vertex.attr")
    ...
    values[1] <- "a"
    g <- set.edge.attribute(g, name="edge.attr", value=values)
    suppressWarnings(checkException(res <- CheckAttributes(g, vertex.attr="vertex.attr", edge.attr="edge.attr"), silent=T))
    g <- remove.edge.attribute(g, "edge.attr")
