test_CreateGraph <- function() {
    ...
    g <- CreateGraph(n=nvertices, gen.vertex.weights=F, binary.pheno=F)
    ...
    g <- CreateGraph(n=nvertices, gen.vertex.weights=T, binary.pheno=T)
    values <- get.vertex.attribute(g, "pheno")
    checkTrue(all(values == 0 | values == 1), T)
    
    # test that CreateGraph generates continuous vertex weights within the range (0,1) when required
    g <- CreateGraph(n=nvertices, gen.vertex.weights=T, binary.pheno=F)
    values <- get.vertex.attribute(g, "pheno")
    checkTrue(max(values) <= 1)
    checkTrue(min(values) >= 0)
    
    # test that CreateGraph successfully adds the correct number of links when multiple clusters are added
    nlinks <- 30
    g <- CreateGraph(n=nvertices, type="barabasi", m=m, gen.vertex.weights=T, nclusters=2, nlinks=nlinks)
