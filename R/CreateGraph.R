CreateGraph <- function(
    ...
    GraphFct <- function(n, type=c("barabasi", "erdos.renyi", "grid"), m, p.or.m, edge.distances) {
        method <- match.arg(type)
        g <- switch(method,
                    barabasi    = simplify(barabasi.game(n, m=m, directed=F)),
                    erdos.renyi = erdos.renyi.game(n, p.or.m=p.or.m, type="gnm", directed=F),
                    grid        = CreateGrid(n)	
        )
    ...
    if (is.null(vertex.weights) & !gen.vertex.weights) {
        ...
        if (nlinks > 0 & nclusters > 1) {
            ...
            for (i in 1:(nclusters - 1)) {
                for (j in (i + 1):nclusters) {
                    g <- simplify(add.edges(g, as.numeric(rbind(sample(id.boundaries[i, 1]:id.boundaries[i, 2], nlinks.binned[c], replace=T), sample(id.boundaries[j, 1]:id.boundaries[j, 2], nlinks.binned[c], replace=T)))))
                    c <- c + 1
                }
            }	
        }
        ...
        g <- set.vertex.attribute(g, name="hits", value=unlist(lapply(graphs, get.vertex.attribute, name="hits")))
        g <- set.vertex.attribute(g, name="hits.cluster", value=unlist(lapply(graphs, get.vertex.attribute, name="hits.cluster")))
        g <- set.vertex.attribute(g, name="color", value=unlist(lapply(graphs, get.vertex.attribute, name="color")))
        ...
        if (is.null(vertex.weights)) {
            ...
            vertex.weights  <- sort(vertex.weights, decreasing=T)
            pheno           <- double(n)
