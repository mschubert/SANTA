plot.Kset <- function(
  ...
  if (all(is.na(x$K.perm), is.na(x$AUK.perm), is.na(x$K.quan))){
    ...
    plot(x$K.obs, main="Kset function", type="l", lwd=3, col="red", xlab="distance on graph", ylab="Kset")
  } else {
    ...
    plot(NA, ylim=range(K.obs, K.quan), xlim=c(0, g.max), xlab="Distance in graph", ylab="Kset", main="Kset-function", ...)
